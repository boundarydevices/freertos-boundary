...
#ifndef EXAMPLE_NOR_CMD_LUT_SEQ_IDX_READID
#define EXAMPLE_NOR_CMD_LUT_SEQ_IDX_READID (NOR_CMD_LUT_SEQ_IDX_READID_OPI)
#endif
...
status_t flexspi_nor_flash_page_program(FLEXSPI_Type *base, uint32_t dstAddr, const uint32_t *src)
{
    ...
    status = flexspi_nor_exec_op(base, dstAddr, FLASH_PORT, kFLEXSPI_Write, NOR_CMD_LUT_SEQ_IDX_PAGEPROGRAM, 1,
...
status_t flexspi_nor_read_data(FLEXSPI_Type *base, uint32_t startAddress, uint32_t *buffer, uint32_t length)
{
    ...
    /* Read page. */
    status =
        flexspi_nor_exec_op(base, startAddress, FLASH_PORT, kFLEXSPI_Read, NOR_CMD_LUT_SEQ_IDX_READ, 1, buffer, length);
