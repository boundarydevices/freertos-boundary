...
void TRDC_InitGlobalConfig(void)
{
        ...
        for (j = 0U; j < hwConfig.domainNumber; j++)
        {
            ...
            for (m = 0U; m < 4; m++)
            {
                ...
                for (n = 0U; n < mbcHwConfig.blockNum; n++)
                {
                    ...
                    { /*mbc0 gpioa and gpiob for non-secure*/
                        if (n == 0 || n == 1)
                        {
                            mbcBlockConfig.nseEnable = true;
                        }
    ...
    // SIM_SEC->SYSCTRL0 |= (SIM_SEC_SYSCTRL0_RGPIOA_TZ_DISABLE(1) | SIM_SEC_SYSCTRL0_RGPIOB_TZ_DISABLE(1));
...
void APP_SetTrdcGlobalConfig(void)
{
    ...
    else
    {
        ...
        /*mbc0 gpioa and gpiob for non-secure*/
        App_SetTrdcMBCNSE(0, 3, 0, 1);
        App_SetTrdcMBCNSE(0, 3, 1, 1);
