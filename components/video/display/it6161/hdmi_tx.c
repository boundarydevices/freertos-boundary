...
void HDMITX_LoadRegSetting(display_handle_t *handle, RegSetTable_t *table, int table_sz)
{
    int i;

    for (i = 0; i < table_sz / (int)sizeof(RegSetTable_t); i++)
    {
...
{
    HDMITX_SetI2C_Byte(handle, 0x0F, 0x03, value & 0x03U);
}
...
static inline bool HDMITX_GetSinkHpd(display_handle_t *handle)
{
    ...
    return tx_sys_status & HDMI_TX_SYS_STATUS_REG0E_RHPDetect_MASK;
}
...
static void HDMITX_AbortDDC(display_handle_t *handle)
{
    ...
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_GENERAL_REG04, SWReset | B_TX_HDCP_RST_HDMITX);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_SYS_DDC_CTRL_REG10, HDMI_TX_SYS_DDC_CTRL_REG10_Reg_MasterSel_MASK);
    ...
    for (i = 0; i < 2U; i++)
    {
        ...
        for (timeout = 0; timeout < 200U; timeout++)
        {
            ...
            if (uc & HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_Done_MASK)
            {
                break; // success
            }
            if (uc & (HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_NoACK_MASK |
                      HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_WaitBus_MASK |
                      HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_ArbiLose_MASK))
            {
                break;
            }
...
static bool getHDMITX_EDIDBytes(display_handle_t *handle, uint8_t *pData, uint8_t bSegment, uint8_t offset, uint16_t Count)
{
    ...
    if (value & HDMI_TX_INT_FLAGS_REG06_RInt_DDCBusHang_MASK)
    {
    ...
    while (RemainedCount > 0U)
    {
        ...
        for (TimeOut = 0; TimeOut < 200U; TimeOut++)
        {
            ...
            if (value & HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_Done_MASK)
            {
                break;
            }
            ...
            if ((value & (HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_NoACK_MASK |
                          HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_WaitBus_MASK |
                          HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_ArbiLose_MASK)) ||
                (ucdata & HDMI_TX_INT_FLAGS_REG06_RInt_DDCBusHang_MASK))
            {
                HDMITX_DEBUG_PRINTF("Called hdmitx_AboutDDC()\n");
                HDMITX_AbortDDC(handle);
                return false;
        ...
        for (TimeOut = 250; TimeOut > 0U; TimeOut--)
        {
            ...
            if (ucdata & HDMI_TX_SYS_DDC_CTRL_REG16_RDDC_Status_Done_MASK)
            {
                break;
            }
...
static bool getHDMITX_EDIDBlock(display_handle_t *handle, int32_t EDIDBlockID, uint8_t *pEDIDData)
{
    ...
    return getHDMITX_EDIDBytes(handle, pEDIDData, EDIDBlockID / 2, (EDIDBlockID % 2) * 128, 128);
}
    ...
    for (offset = 4; offset < End;)
    {
        ...
        switch (tag)
        {
            ...
            case 0x02: // Video Data Block ;
                offset++;
                for (i = 0U, RxCapability.NativeVDOMode = 0xff; i < count; i++)
                {
                    uint8_t VIC = 0U;
                    ...
                    if (VIC < 64U)
                    {
                        RxCapability.VDOMode[VIC / 8U] |= (1 << (VIC % 8));
                        HDMITX_DEBUG_PRINTF("VIC = %d, RxCapability.VDOMode[%d]=%02X\n", (int)VIC, (int)VIC / 8,
                                            (int)RxCapability.VDOMode[VIC / 8]);
                        if ((pCEAEDID[offset + i] & 0x80U) && (RxCapability.NativeVDOMode == 0xFF))
                        {
            ...
            case 0x03: // Vendor Specific Data Block ;
                ...
                if (RxCapability.IEEEOUI == 0x0C03)
                {
                    ...
                    {
                        nextoffset = 8;
                        if (pCEAEDID[offset + 7] & 0x80U)
                        {
                            nextoffset += 2;
                        } // latency
                        if (pCEAEDID[offset + 7] & 0x40U)
                        {
                            nextoffset += 2;
                        } // interlace latency
                        if (pCEAEDID[offset + 7] & 0x20U)
                        {
...
static uint8_t ParseEDID(display_handle_t *handle)
{
    ...
    {
        err = getHDMITX_EDIDBlock(handle, i, EDID_Buf);
        ...
        if (err)
        {
            if ((!RxCapability.ValidHDMI) && EDID_Buf[0] == 0x2U && EDID_Buf[1] == 0x3U)
            {
...
static bool HDMITX_GetVideoState(display_handle_t *handle)
{
    ...
    return HDMI_TX_SYS_STATUS_REG0E_TxVidStable_MASK & value;
}

static int32_t HDMITX_CalcRclk(display_handle_t *handle)
{
    int i   = 0;
    int sum = 0, RCLKCNT = 0, TimeLoMax = 0, retry = 5;
    uint8_t value = 0U;
    ...
    sum /= retry;
    RCLKCNT = sum / 1000;
    HDMITX_CEC_WriteI2C_Byte(handle, 0x0C, (RCLKCNT & 0xFF));
    ...
    HDMITX_WriteI2C_Byte(handle, 0x47, (TimeLoMax & 0xFF));
    HDMITX_WriteI2C_Byte(handle, 0x48, ((TimeLoMax & 0xFF00) >> 8));
    HDMITX_WriteI2C_Byte(handle, 0x49, 0x23);

    return it6161.hdmi_tx.rclk;
}
...
int32_t HDMITX_CalcPclk(display_handle_t *handle)
{
    ...
    HDMITX_SetI2C_Byte(handle, 0xD7, 0x70, div << 4);

    HDMITX_ReadI2C_Byte(handle, 0xD7, &value);
    uc = value & 0x7F;
    for (i = 0, sum = 0; i < 100; i++)
    {
        HDMITX_WriteI2C_Byte(handle, 0xD7, uc | 0x80);
        delay1ms(1);
    ...
    it6161.hdmi_tx.pclk = it6161.hdmi_tx.rclk * 128 / count * 16; /* 128*16=2048 */
    it6161.hdmi_tx.pclk *= (1 << div);

    HDMITX_DEBUG_PRINTF("hdmi tx pclk = %d.%d MHz", it6161.hdmi_tx.pclk / 1000, it6161.hdmi_tx.pclk % 1000);
    return it6161.hdmi_tx.pclk;
}

static void HDMITX_GetDisplayMode(display_handle_t *handle)
{
    int32_t interlaced = 0;
    int32_t htotal = 0, hdes = 0, hdee = 0, hsyncw = 0, H2ndVRRise = 0;
    int32_t vtotal = 0, vdes = 0, vdee = 0, vdes2nd = 0, vdee2nd = 0;
    int32_t VRS2nd = 0;
    uint8_t rega9  = 0;
    uint8_t value  = 0U;
...
int32_t HDMI_AviInfoframePack(struct hdmi_avi_infoframe *frame, void *buffer, int32_t size)
{
    ...
    if (frame->type != HDMI_INFOFRAME_TYPE_AVI || frame->version != 2 || frame->length != HDMI_AVI_INFOFRAME_SIZE)
        return -1;
    ...
    ptr[1] = ((frame->colorimetry & 0x3) << 6) | ((frame->picture_aspect & 0x3) << 4) | (frame->active_aspect & 0xF);

    ptr[2] =
        ((frame->extended_colorimetry & 0x7) << 4) | ((frame->quantization_range & 0x3) << 2) | (frame->nups & 0x3);
    ...
    ptr[4] =
        ((frame->ycc_quantization_range & 0x3) << 6) | ((frame->content_type & 0x3) << 2) | (frame->pixel_repeat & 0xF);
...
static int32_t HDMITX_AviInfoframeSet(display_handle_t *handle)
{
    ...
    ret = HDMI_AviInfoframePack(&it6161.source_avi_infoframe, buf, sizeof(buf));
    if (ret < 0)
    {
    ...
    for (i = 0; i < it6161.source_avi_infoframe.length; i++)
        HDMITX_WriteI2C_Byte(handle, HDMI_TX_AVIINFO_DB1 + i, ptr[i]);
...
int32_t HDMI_AudioInfoframePack(struct hdmi_audio_infoframe *frame, void *buffer, int32_t size)
{
    ...
    if (frame->type != HDMI_INFOFRAME_TYPE_AUDIO || frame->version != 1 || frame->length != HDMI_AUDIO_INFOFRAME_SIZE)
        return -1;
    ...
    ptr[1] = ((frame->sample_frequency & 0x7) << 2) | (frame->sample_size & 0x3);
...
static void HDMITX_AudioInfoframeSet(display_handle_t *handle, uint8_t channels)
{
    ...
    ret = HDMI_AudioInfoframePack(&frame, buf, sizeof(buf));
    if (ret < 0)
    {
...
static void setHDMITX_ChStat(display_handle_t *handle, uint8_t ucIEC60958ChStat[])
{
    ...
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDCHST_SRCNUM, ucIEC60958ChStat[2] & 0xF);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUD0CHST_CHTNUM, (ucIEC60958ChStat[2] >> 4) & 0xF);
...
static void setHDMITX_LPCMAudio(display_handle_t *handle, uint8_t AudioSrcNum, uint8_t AudSWL, uint8_t bAudInterface)
{
    ...
    else
    {
        ...
        switch (AudioSrcNum)
        {
            ...
            case 3:
                AudioEnable |= 2 | HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudioEn_Enable_Audio_Source_1 |
                               HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudioEn_Enable_Audio_Source_0;
                break;

            case 2:
                AudioEnable |= 1 | HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudioEn_Enable_Audio_Source_0;
                break;
    ...
    HDMITX_ChangeBank(handle, 0);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDIO_CHANNEL_REGE0, AudioEnable & 0xF0);
    ...
    if (bAudInterface == AUDIO_IF_SPDIF)
    {
        ...
        HDMITX_SetI2C_Byte(handle, 0x5c, (1 << 6), (1 << 6));
        for (i = 0; i < 100; i++)
        {
            HDMITX_ReadI2C_Byte(handle, HDMI_TX_CLOCK_CONTROL_REG5F, &value);
            if (value & HDMI_TX_CLOCK_CONTROL_REG5F_OSFreqLock_MASK)
            {
                break; /* stable clock. */
            }
        }
...
static void setHDMITX_NLPCMAudio(display_handle_t *handle, uint8_t bAudInterface)
{
    ...
    else
    {
        ...
        value &= ~HDMI_TX_AUDIO_CHANNEL_REGE5_RegEnTDM_MASK;
        /* 2 channel NLPCM, no TDM mode. */
        HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDIO_CHANNEL_REGE5, value);
    }
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDIO_CHANNEL_REGE0, AudioEnable | 0);
}
...
static void setHDMITX_HBRAudio(display_handle_t *handle, uint8_t bAudInterface)
{
    ...
    {
        HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDIO_CHANNEL_REGE0,
                             HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudSWL_24bits | 3 |
                                 HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudioEn_Enable_Audio_Source_2 | 1 |
                                 HDMI_TX_AUDIO_CHANNEL_REGE0_REGAudioEn_Enable_Audio_Source_0);
    }
    HDMITX_SetI2C_Byte(handle, 0x5c, 1 << 6, 0x00);
...
static void setHDMITX_NCTS(display_handle_t *handle, uint8_t Fs)
{
    ...
    HDMITX_ReadI2C_Byte(handle, HDMI_TX_AUDIO_CHANNEL_REGE5, &value);
    if (HDMI_TX_AUDIO_CHANNEL_REGE5_RegAudHBR_MASK & value)
    {
        HBR_mode = true;
    }
    else
    ...
    HDMITX_ChangeBank(handle, 1);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudN0, (uint8_t)((n)&0xFF));
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudN1, (uint8_t)((n >> 8) & 0xFF));
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudN2, (uint8_t)((n >> 16) & 0xF));

    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudCTS0, (uint8_t)((LastCTS)&0xFF));
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudCTS1, (uint8_t)((LastCTS >> 8) & 0xFF));
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_REGPktAudCTS2, (uint8_t)((LastCTS >> 16) & 0xF));
    HDMITX_ChangeBank(handle, 0);
    ...
    if (false == HBR_mode)
    {
        ...
        HDMITX_ChangeBank(handle, 1);
        Fs = AUDFS_768KHz;
        HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDCHST_CA_FS, 0x00 | Fs);
        /* OFS is the one's complement of FS */
        Fs = ~Fs;

        HDMITX_ReadI2C_Byte(handle, HDMI_TX_AUDCHST_OFS_WL, &value);
        uData = (0x0f & value);
        HDMITX_WriteI2C_Byte(handle, HDMI_TX_AUDCHST_OFS_WL, (Fs << 4) | uData);
        HDMITX_ChangeBank(handle, 0);
    }
}
                                     ...
                                     uint8_t *pIEC60958ChStat)
{
    ...
    HDMITX_WriteI2C_Byte(
        ...
        HDMI_TX_CLOCK_CONTROL_REG58_REGAutoOSCLK_MASK | HDMI_TX_CLOCK_CONTROL_REG58_REGMCLKFreq_2x128Fs | 0x01);
    ...
    switch (AudioType)
    {
        ...
        case T_AUDIO_LPCM:
            ...
            setHDMITX_ChStat(handle, pIEC60958ChStat);
            setHDMITX_LPCMAudio(handle, (ChNum + 1) / 2, SUPPORT_AUDI_AudSWL, bAudInterface);
            break;
    }
...
{
    if (it6161.hdmi_tx.support_audio)
    {
...
static void HDMITX_SetAvMute(display_handle_t *handle, uint8_t bEnable)
{
    ...
    HDMITX_SetI2C_Byte(handle, HDMI_TX_HDMI_CONTROL_REGC1, HDMI_TX_HDMI_CONTROL_REGC1_REGAVMute_MASK,
                       bEnable ? HDMI_TX_HDMI_CONTROL_REGC1_REGAVMute_MASK : 0);
    HDMITX_WriteI2C_Byte(
...
static void HDMITX_SetupCsc(display_handle_t *handle)
{
    ...
    else
    {
        if (ptable != NULL)
        {
            ...
            {
                HDMITX_WriteI2C_Byte(handle, HDMI_TX_COLOR_SPACE_CONVERSION_REG73 + i, ptable[i]);
            }
        }
...
static void HDMITX_AbortDdc(display_handle_t *handle)
{
    ...
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_GENERAL_REG04, sw_reset | B_TX_HDCP_RST_HDMITX);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_SYS_DDC_CTRL_REG10, HDMI_TX_SYS_DDC_CTRL_REG10_Reg_MasterSel_MASK);
...
static void HDMITX_ClrInt(display_handle_t *handle, uint8_t Reg06, uint8_t Reg07, uint8_t Reg08, uint8_t RegEE)
{
    ...
    if (Reg06 & HDMI_TX_INT_FLAGS_REG06_RInt_AudioOvFlwStus_MASK)
    {
    ...
    else if (Reg06 & HDMI_TX_INT_FLAGS_REG06_RInt_DDCFIFOErr_MASK)
    {
        HDMITX_DEBUG_PRINTF("DDC FIFO Error");
        HDMITX_ClrDdcFifo(handle);
    }
    else if (Reg06 & HDMI_TX_INT_FLAGS_REG06_RInt_DDCBusHang_MASK)
    {
...
{
    if (Reg06 & HDMI_TX_INT_FLAGS_REG06_RInt_HPDStus_MASK)
    {
...
static void HDMITX_EnableVideoOutput(display_handle_t *handle, uint8_t level)
{
    ...
    if (it6161.hdmi_tx.hdmi_mode)
    {
        HDMITX_SetAvMute(handle, true);
    }

    HDMITX_SetupPclkDiv2(handle);
    HDMITX_SetupCsc(handle);
    HDMITX_WriteI2C_Byte(handle, HDMI_TX_HDMI_CONTROL_REGC0,
                         it6161.hdmi_tx.hdmi_mode ? HDMI_TX_HDMI_CONTROL_REGC0_REGHDMIMode_HDMI_MODE :
...
static void HDMITX_SetOutputProcess(display_handle_t *handle)
{
    ...
    TMDSClock = it6161.hdmi_tx.pclk * 1000 * (it6161.source_avi_infoframe.pixel_repeat + 1);
    ...
    HDMITX_SetAvMute(handle, false);
}

void HDMITX_Reg08_Process(display_handle_t *handle, uint8_t Reg08)
{
    if (Reg08 & HDMI_TX_INT_FLAGS_REG08_RInt_VidStableStus_MASK)
    {
